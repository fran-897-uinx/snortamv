...
def create_account(username: str, fullname: str, password: str) -> bool:
    ...
    try:
        cur.execute(
            "INSERT INTO accounts (username, fullname, password_hash) VALUES (?, ?, ?)",
...
def get_account(username: str) -> Optional[Dict[str, Any]]:
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, username, fullname, created_at FROM accounts WHERE username = ?", (username,))
    row = cur.fetchone()
    conn.close()
...
def update_account(username: str, new_fullname: Optional[str] = None, new_password: Optional[str] = None) -> bool:
    conn = get_connection()
    cur = conn.cursor()
    # ensure user exists
    ...
    if new_fullname and new_password:
        ...
        cur.execute("UPDATE accounts SET fullname = ? WHERE username = ?", (new_fullname, username))
    elif new_password:
        cur.execute("UPDATE accounts SET password_hash = ? WHERE username = ?", (hash_password(new_password), username))
    else:
        conn.close()
        return False
    conn.commit()
...
def migrate_from_json(project_root: Path) -> int:
    ...
    for username, info in data.items():
        ...
        if not username or not password:
            continue
        created = create_account(username, fullname, password)
        if created:
            count += 1
